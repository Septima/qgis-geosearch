...
class SettingManager(QObject):
    ...
    def value(self, setting_name):
        if setting_name not in self.__settings:
            raise NameError(
                "%s has no setting %s" % (self.plugin_name, setting_name)
            )
    ...
    def remove(self, setting_name):
        if setting_name not in self.__settings:
            raise NameError(
                "{} has no setting {}".format(self.plugin_name, setting_name)
            )
    ...
    def addSetting(self, name, setting_type, tscope, default_value, options={}):
        print(
            "qgissettingmanager:: calling addSetting with these chain of argument is deprecated."
            " Consider using add_setting."
        )
        if name in self.__settings is not None:
            raise NameError("%s already exist in settings." % name)
        if setting_type.lower() not in (
            "string",
            "double",
            "integer",
            "bool",
        ...
        if tscope.lower() == "global":
            ...
            raise NameError(
                "%s is not a valid scope. Must be project or global." % tscope
            )
        SettingClass = globals()[
            setting_type[0].upper() + setting_type[1:].lower()
        ]
    ...
    def setValue(self, setting_name, value):
        print(
            "qgissettingmanager:: calling setValue is deprecated. Consider using set_value."
        )
