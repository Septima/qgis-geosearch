...
class SearchBox(QFrame, FORM_CLASS):
    ...
    def readconfig(self):
        ...
        self.config = {
            ...
            'muncodes':      muncodes,
            'rubber_color':  str(s.value(k + "/rubberband/color", str(s.value(k + "/rubber_color", "#FF0000", type=str)), type=str)), # Hack for getting original ver. 0.27 setting 
            'rubber_width':  s.value(k + "/rubberband/width",     s.value(k + "/rubber_width", 4, type=int), type=int),
            'rubber_style':  s.value(k + "/rubberband/style",     s.value(k + "/rubber_style", 1, type=int), type=int),
            'marker_color':  str(s.value(k + "/marker/color",     str(s.value(k + "/marker_color", "#FF0000", type=str)), type=str)),
            'marker_icon':   s.value(k + "/marker/icon",          s.value(k + "/marker_icon", QgsVertexMarker.ICON_CROSS, type=int), type=int),
            'marker_width':  s.value(k + "/marker/width",         s.value(k + "/marker_width", 4, type=int), type=int),
            'marker_size':   s.value(k + "/marker/size",          s.value(k + "/marker_size", 30, type=int), type=int),
            'marker_buffer': s.value(k + "/marker/buffer",        s.value(k + "/marker_buffer", 199.99, type=float), type=float),
            'rubber_buffer': s.value(k + "/rubberband/buffer",    s.value(k + "/rubber_buffer", 199.99, type=float), type=float)
        }
